...
public class currencyconversioncontroller {
	...
	// Feign-way-of-calling-micro-service-from-another-service
	
	@GetMapping("currency-conversion-feign/from/{from}/to/{to}/quantity/{quantity}")
	public CurrencyConversion retrieveExchangevalueFeign(
			@PathVariable("from") String fromCurrency,
			@PathVariable("to") String toCurrency,
			@PathVariable BigDecimal quantity) {

		CurrencyConversion currencyConversion = proxy.retrieveExchangevalue(fromCurrency, toCurrency);
		return new CurrencyConversion(
				currencyConversion.getId(), fromCurrency, toCurrency, quantity, 
				currencyConversion.getConversionMultiple(), 
				quantity.multiply(currencyConversion.getConversionMultiple()), 
